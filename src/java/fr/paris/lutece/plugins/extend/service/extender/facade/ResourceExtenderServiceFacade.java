...
public class ResourceExtenderServiceFacade
{
    ...
    public static String getInfoForExport( String extenderType, String strIdExtendableResource, String strExtendableResourceType ) throws InfoExtenderException
    {
        ...
        return _listExtenderType.stream( ).filter( type -> type.getType( ).equals( extenderType ) ).findAny( ).orElseThrow( InfoExtenderException::new )
                .getInfoForExport( strIdExtendableResource, strExtendableResourceType );
    ...
    public static String getInfoForRecap( String extenderType, String strIdExtendableResource, String strExtendableResourceType ) throws InfoExtenderException
    {
        ...
        return _listExtenderType.stream( ).filter( type -> type.getType( ).equals( extenderType ) ).findAny( ).orElseThrow( InfoExtenderException::new )
                .getInfoForRecap( strIdExtendableResource, strExtendableResourceType );
    ...
    public static <T extends IExtendableResourceResult> List<T> getInfoExtender( String extenderType, String strIdExtendableResource,
            String strExtendableResourceType ) throws InfoExtenderException
    {

        return (List<T>) _listExtenderType.stream( ).filter( type -> type.getType( ).equals( extenderType ) ).findAny( )
                .orElseThrow( InfoExtenderException::new ).getInfoExtender( strIdExtendableResource, strExtendableResourceType );
    }
     ...
     */
    @SuppressWarnings( "unchecked" )
    public <T extends IExtendableResourceResult> List<T> getInfoExtenderByList( String extenderType, List<String> listIdExtendableResource,
            String strExtendableResourceType ) throws InfoExtenderException
    {

        return (List<T>) _listExtenderType.stream( ).filter( type -> type.getType( ).equals( extenderType ) ).findAny( )
                .orElseThrow( InfoExtenderException::new ).getInfoExtenderByList( listIdExtendableResource, strExtendableResourceType );
    ...
    public static <T extends IExtendableResourceResult> Optional<ExtenderType<T>> getExtenderType( Class<T> clazz )
    {
        ...
        return Optional.ofNullable(
                (ExtenderType<T>) _listExtenderType.stream( ).filter( tp -> tp.getClassType( ).isAssignableFrom( clazz ) ).findAny( ).orElse( null ) );
    }
     ...
     * 
     * @param extenderType
     *            the ExtenderType<T> (example: ExtenderType<Hit> or ExtenderType<Rating> or ... )
     * @param ?
     *            the extenderType information object (example: Hit)
    ...
    public static <T extends IExtendableResourceResult> boolean addExtenderType( final ExtenderType<T> extenderType )
    {
        ...
        if ( extenderType != null && extenderType.getType( ) != null
                && _listExtenderType.stream( ).noneMatch( type -> type.getClassType( ).isAssignableFrom( extenderType.getClassType( ) )
                        || extenderType.getClassType( ).isAssignableFrom( type.getClassType( ) ) || type.getType( ).equals( extenderType.getType( ) ) ) )
        {
    ...
    private static List<ExtenderType<? extends IExtendableResourceResult>> initExtenderType( )
    {
        ...
        return new ArrayList<>( Arrays.asList( new ExtenderType<>( Hit.class, HitResourceExtender.EXTENDER_TYPE,
                ( strIdExtendableResource, strExtendableResourceType ) -> HitHome.findByParameters( strIdExtendableResource, strExtendableResourceType )
                        .map( Collections::singletonList ).orElseGet( Collections::emptyList ),
                HitHome::findByResourceList,
                ( strIdExtendableResource, strExtendableResourceType ) -> HitHome.findByParameters( strIdExtendableResource, strExtendableResourceType )
                        .map( hit -> hit != null ? String.valueOf( hit.getNbHits( ) ) : null ).orElse( null ),
                ( strIdExtendableResource, strExtendableResourceType ) -> HitHome.findByParameters( strIdExtendableResource, strExtendableResourceType )
                        .map( hit -> hit != null ? String.valueOf( hit.getNbHits( ) ) : null ).orElse( null )
